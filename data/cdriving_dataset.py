...
class CDriving_loader(BaseDataset):
    colors = [  # [  0,   0,   0],
        [128, 64, 128],
        [244, 35, 232],
        [70, 70, 70],
    ...
    def __init__(self, opt, logger, augmentations=None, split='train'):
        ...
        self.annotations_base = os.path.join(self.root, self.split, 'compound')

        if opt.open and self.split == 'val':
            self.images_base = os.path.join(self.root, self.split, 'open')
            self.annotations_base = os.path.join(self.root, self.split, 'open')
    ...
    def __getitem__(self, index):
        ...
        lbl_path = os.path.join(
            ...
            os.path.basename(img_path)[:-4] + "_train_id.png")
            ...
            if self.opt.proto_rectify:
                ...
                if self.opt.threshold:
                    conf = np.load(os.path.join(
                            self.opt.path_LP,
                            os.path.basename(os.path.dirname(img_path)),
                            os.path.basename(img_path).replace('.jpg', '_conf.npy')))
