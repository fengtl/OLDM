...
class RandomSized(object):
    ...
    def __call__(self, img, mask, mask1=None, lpsoft=None, params=None):
        ...
        w = int(random.uniform(0.75, 1.25) * self.size)
        #w = self.size
        h = int(w/prop)
...
class ColorJitter(object):
    ...
    def __call__(self, img, mask=None, mask1=None, lpsoft=None, params=None):
        out = img
        ...
        return out, mask, mask1, lpsoft, params
...
class GaussianBlur(object):
    ...
    def __call__(self, img, mask=None, mask1=None, lpsoft=None, params=None):
        ...
        kernel_size_y = int(np.floor(np.ceil(0.1 * img.size[0]) - 0.5 + np.ceil(0.1 * img.size[0]) % 2))
        kernel_size_x = int(np.floor(np.ceil(0.1 * img.size[1]) - 0.5 + np.ceil(0.1 * img.size[1]) % 2))
