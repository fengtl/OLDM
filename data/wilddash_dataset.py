...
def find_files(root_file='.'):
    return_list = []
    with open(root_file, 'r') as f:
        for line in f:
            return_list.append(line[:-1] + '.jpg')
    return return_list


class WildDash_loader(BaseDataset):
    colors = [  # [  0,   0,   0],
        [128, 64, 128],
        [244, 35, 232],
        [70, 70, 70],
    ...
    def __init__(self, opt, logger, augmentations=None, split='val'):
        ...
        self.annotations_base = os.path.join(self.root, 'semantic_label')

        self.files = sorted(find_files(root_file=os.path.join(self.root, 'random_split/validation.txt')))
    ...
    def __getitem__(self, index):
        ...
        :param index:
        """
        img_path = os.path.join(self.images_base, self.files[index].rstrip())
        ...
        lbl_path = os.path.join(
            ...
            os.path.basename(img_path)[:-4] + "_labelIds.png")
